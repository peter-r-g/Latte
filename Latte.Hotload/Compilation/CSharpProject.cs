...
using System.Linq;
using System.Xml;
using Latte.Hotload.Extensions;
...
{
	/// <summary>
	/// The absolute path to the csproj file that this was created from.
	/// </summary>
	...
	/// </summary>
	private static Dictionary<string, CSharpProject> FileCache { get; } = new();
	...
	private CSharpProject( string filePath )
	{
		...
		if ( FileCache.ContainsKey( filePath ) )
			FileCache[filePath] = this;
		else
			FileCache.Add( filePath, this );
	}
	...
	private static bool AnalyzeCondition( string condition )
	{
		...
		condition = condition.Replace( "$(Configuration)", CompilerHelper.Build ).Replace( "'", "" );
	...
	{
		if ( FileCache.TryGetValue( filePath, out var csproj ) )
			return csproj;
	...
	{
		return FileCache.Remove( filePath );
	}
